	...
	public override void OnInspectorGUI() {
        ...
        if (imageFileList != null && ARToolKitAssetManager.Images != null && imageFileList.Length != ARToolKitAssetManager.Images.Length) {
            ...
            if (imageFileList.Length < ARToolKitAssetManager.Images.Length) {
                ...
                Debug.logger.Log (LogType.Warning, "<color=red>Trackable image removed. Please check all ARTrackables and make sure that they have an image assigned.</color>");
            }
        ...
        if (arMarker.EditorMarkerIndex < 0) {
            ...
            Debug.logger.Log (LogType.Warning, "<color=red>Trackable image removed. Please check the ARTrackable and make sure that is has an image assigned.</color>");
            return;
        }
        else{
    ...
    private void DropAreaGUI ()
    {
        ...
        case EventType.DragPerform:
            if (!dropArea.Contains (currentEvent.mousePosition)) {
                Debug.logger.Log (LogType.Warning, "Drop released outside the area" + currentEvent.mousePosition);
                return;
            }
